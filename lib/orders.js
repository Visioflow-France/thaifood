...
// LIBÈRE un numéro quotidien : la commande qui l'avait pris a échoué (paiement
// refusé) ou été annulée — le numéro ne doit PAS lui appartenir. Si c'était le
// DERNIER numéro attribué du jour, on décrémente le compteur : la prochaine
// commande effective récupère ce numéro (pas de trou dans la numérotation).
// Si des numéros plus récents ont déjà été attribués entre-temps, on ne peut
// pas décrémenter (sinon doublon) — le trou est alors assumé (rare).
export async function releaseDailySequence(day, seq) {
  if (!day || !Number.isFinite(seq)) return;
  const counterRef = getDb().collection('config').doc('orderSeq');
  const today = todayKey();
  if (day !== today) return; // jour passé : le compteur s'est déjà réinitialisé
  await getDb().runTransaction(async (tx) => {
    const snap = await tx.get(counterRef);
    const data = snap.exists ? snap.data() : null;
    if (!data || data.date !== today) return; // jour changé entre-temps
    if (Number(data.count) !== seq) return; // pas le dernier numéro → trou assumé
    tx.set(counterRef, { date: today, count: Math.max(0, seq - 1) }, { merge: true });
  });
}
