...
// Minutes depuis minuit (Paris) d'un instant donné.
function parisMinutes(iso) {
  ...
  return m ? (Number(m[2]) % 24) * 60 + Number(m[3]) : 0;
}

// Bascule midi → soir : la numérotation repart à 1 au début du service du soir.
const EVENING_START = 15 * 60; // 15h00 : fin du service midi

// Clé du SERVICE courant : « YYYY-MM-DD#midi » ou « YYYY-MM-DD#soir ». Le numéro
// de commande se réinitialise à chaque changement de service (pas seulement de
// jour) : midi compte 1, 2, 3… puis le soir REPART à 1, 2, 3…
export function serviceKey(iso) {
  const day = todayKey();
  const mins = parisMinutes(iso);
  return `${day}#${mins < EVENING_START ? 'midi' : 'soir'}`;
}

// Numéro de commande séquentiel (1, 2, 3…) via un compteur Firestore
// `config/orderSeq` mis à jour en transaction (atomique, sans doublon).
// Se réinitialise à 1 À CHAQUE SERVICE (midi / soir) : la clé du compteur
// contient le service, donc le soir reprend à 1 naturellement.
export async function nextDailySequence() {
  return getDb().runTransaction((tx) => nextDailySequenceTx(tx));
}

// Même logique, mais À L'INTÉRIEUR d'une transaction existante (utilisé par
// markOrderPaid pour que commande + compteur soient écrits atomiquement —
// voir le commentaire sur markOrderPaid).
async function nextDailySequenceTx(tx) {
  const counterRef = getDb().collection('config').doc('orderSeq');
  const key = serviceKey();
  const snap = await tx.get(counterRef);
  const data = snap.exists ? snap.data() : null;
  const seq = !data || data.key !== key ? 1 : (Number(data.count) || 0) + 1;
  tx.set(counterRef, { key, count: seq }, { merge: true });
  return { day: key, seq };
}

// LIBÈRE un numéro : la commande qui l'avait pris a échoué (paiement refusé) ou
// été annulée — le numéro ne doit PAS lui appartenir. Si c'était le DERNIER
// numéro attribué du service, on décrémente le compteur : la prochaine commande
// effective récupère ce numéro (pas de trou). Si des numéros plus récents ont
// déjà été attribués entre-temps, on ne peut pas décrémenter (sinon doublon) —
// le trou est alors assumé (rare).
export async function releaseDailySequence(day, seq) {
  if (!day || !Number.isFinite(seq)) return;
  const counterRef = getDb().collection('config').doc('orderSeq');
  const key = serviceKey();
  if (day !== key) return; // service passé : le compteur s'est déjà réinitialisé
  await getDb().runTransaction(async (tx) => {
    const snap = await tx.get(counterRef);
    const data = snap.exists ? snap.data() : null;
    if (!data || data.key !== key) return; // service changé entre-temps
    if (Number(data.count) !== seq) return; // pas le dernier numéro → trou assumé
    tx.set(counterRef, { key, count: Math.max(0, seq - 1) }, { merge: true });
  });
}
...
export async function getOrders() {
  try {
    ...
    const list = snap.empty ? [] : snap.docs.map((d) => ({ id: d.id, ...d.data() }));
    // FILET DE SÉCURITÉ : une commande « awaiting_payment » depuis plus de
    // 2 minutes alors que le client est revenu de Stripe = webhook perdu
    // (bug, panne Stripe, endpoint mal configuré…). On rattrape AVANT de
    // renvoyer la liste : la commande payée apparaît dans le dashboard quoi
    // qu'il arrive. Voir recoverPendingPayments.
    await recoverPendingPayments(list);
    // Relecture si au moins une commande a été rattrapée (statuts frais).
    const recovered = list.some(
      (o) => o.status === 'awaiting_payment' && o._recovered
    );
    if (recovered) {
      const fresh = await getDb().collection(COL_ORDERS).orderBy('createdAt', 'desc').get();
      return fresh.empty ? list : fresh.docs.map((d) => ({ id: d.id, ...d.data() }));
    }
    return list;
  } catch (e) {
...
// Rattrapage anti « commande payée mais invisible » : pour chaque commande
// récente (moins de 24 h) encore en 'awaiting_payment' après 2 minutes
// (marge pour laisser le webhook arriver normalement), on interroge Stripe
// (session de checkout). Si Stripe dit « paid » → markOrderPaid (même code
// que le webhook). Les échecs sont silencieux : ce filet ne doit JAMAIS
// casser l'affichage du dashboard.
// Coût : 1 appel Stripe par commande coincée, uniquement quand ça arrive.
async function recoverPendingPayments(orders) {
  for (const o of orders) {
    if (!o || o.status !== 'awaiting_payment') continue;
    // Marge de 2 min : le webhook met normalement quelques secondes — inutile
    // d'interroger Stripe pendant ce laps de temps.
    if (Date.now() - new Date(o.createdAt || 0).getTime() < 2 * 60 * 1000) continue;
    const recovered = await recoverOrderIfNeeded(o);
    if (recovered) o._recovered = true;
  }
}

// Version pour UNE commande (page de suivi client /commande) : pas de délai
// minimum — le client vient juste de payer, on veut la confirmation immédiate
// si le webhook traîne. Renvoie la commande mise à jour, ou null si rien à faire.
export async function recoverOrderIfNeeded(order) {
  if (!order || order.status !== 'awaiting_payment') return null;
  try {
    const { getStripe } = await import('./stripe');
    const stripe = getStripe();
    if (!stripe) return null;
    const cutoff = Date.now() - 24 * 3600 * 1000; // on ne rattrape pas l'historique ancien
    if (!order.createdAt || new Date(order.createdAt).getTime() <= cutoff) return null;

    let session = null;
    if (order.checkoutSessionId) {
      session = await stripe.checkout.sessions.retrieve(order.checkoutSessionId);
    } else {
      // Pas d'ID enregistré (vieille commande) : on cherche par métadonnée.
      const found = await stripe.checkout.sessions.list({ limit: 20 });
      session = found.data.find((s) => s.metadata?.ref === order.ref) || null;
    }
    if (session && session.payment_status === 'paid') {
      const updated = await markOrderPaid(order.ref, {
        paymentIntentId: session.payment_intent || null,
        checkoutSessionId: session.id || null,
        paidAmount: session.amount_total ?? null,
        feeAmount: session.total_details?.application_fee?.amount ?? null,
      });
      console.log('[orders] rattrapage :', order.ref, 'marquée payée (webhook perdu)');
      return updated;
    }
  } catch (e) {
    console.error('[orders] rattrapage impossible pour', order?.ref, ':', e?.message);
  }
  return null;
}
...
export async function updateOrderStatus(ref, status) {
  ...
  // Annulation d'une commande numérotée : le numéro est libéré pour la
  // prochaine commande effective (règle : un numéro = une commande réelle).
  if (status === 'cancelled' && Number.isFinite(found.order.dailySeq)) {
    await releaseDailySequence(found.order.day, found.order.dailySeq);
  }
...
export async function markOrderPaid(ref, payment = {}) {
  ...
  // ⚠️ La commande et son numéro sont écrits DANS UNE TRANSACTION. Stripe
  // envoie plusieurs événements pour un même paiement (checkout.session.completed
  // + payment_intent.succeeded) : sans transaction, deux webhooks concurrents
  // passaient tous deux le test « déjà payée ? » et incrémentaient le compteur
  // deux fois → la 1re commande du jour se retrouvait numérotée n°2. La
  // transaction sérialise : le 2e webhook relit la commande déjà 'paid' et sort.
  const patch = {
    status: 'paid',
  ...
  const result = await getDb().runTransaction(async (tx) => {
    const snap = await tx.get(found.doc);
    const current = snap.exists ? snap.data() : null;
    if (!current || current.status === 'paid' || current.status === 'cancelled') return null;
    ...
    if (!Number.isFinite(current.dailySeq)) {
      const { day, seq } = await nextDailySequenceTx(tx);
      patch.day = day;
      patch.dailySeq = seq;
    }
    tx.update(found.doc, patch);
    return { ...current, ...patch };
  });
  // null = un webhook concurrent l'a déjà traitée : on relit l'état final.
  return result || (await getOrderByRef(ref));
}

// Marque une commande en échec de paiement (appelé par le webhook Stripe).
// Le numéro quotidien est LIBÉRÉ : la commande ne l'a jamais vraiment « mérité »
// (le paiement a échoué, la cuisine ne la préparera pas).
