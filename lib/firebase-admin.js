...
// Sous Cloudflare Workers (workerd), gRPC n'existe pas : Firestore doit passer
// par son API REST (fetch). Sans ça, la 1re requête fait planter le Worker
// (500 à corps vide, l'erreur gRPC échappe aux try/catch des routes).
export const IS_FIRESTORE_REST =
...
export function getDb() {
  if (!_db) {
    ...
    if (IS_FIRESTORE_REST) db.settings({ preferRest: true });
    _db = db;
  }
  return _db;
}
